...
TEST(OrderStatisticSetTest, SetPublicTypes) {
  ...
  Set s;
  static_assert(std::is_same_v<size_t, Set::key_type>);
  static_assert(!std::is_same_v<std::string, Set::key_type>);
  static_assert(std::is_same_v<size_t, Set::value_type>);
  static_assert(std::is_same_v<size_t, Set::size_type>);
  static_assert(std::is_same_v<size_t&, Set::reference>);
  static_assert(std::is_same_v<const size_t&, Set::const_reference>);
  static_assert(!is_transparent<std::less<int>>::value);
  static_assert(is_transparent<std::less<>>::value);
  static_assert(is_transparent<Set::key_compare>::value);
  static_assert(!is_transparent<
                OrderStatisticSet<size_t, std::less<int>>::key_compare>::value);
  static_assert(is_transparent<Set::value_compare>::value);
}
...
TEST(OrderStatisticSetTest, IteratorPublicTypes) {
  ...
  static_assert(std::is_same_v<size_t, It::value_type>);
  static_assert(std::is_same_v<ptrdiff_t, It::difference_type>);
  static_assert(std::is_same_v<size_t*, It::pointer>);
  static_assert(std::is_same_v<size_t&, It::reference>);
}
...
TEST(OrderStatisticSetTest, Basic) {
  ...
  EXPECT_THAT(ost.insert(1),
              Pair(AllOf(IteratorReferences(&ost, 1), Rank(0)), true));
  EXPECT_EQ(set, ost);
  EXPECT_THAT(ost.lower_bound(0), IteratorReferences(&ost, 1));
  EXPECT_THAT(ost.lower_bound(1), IteratorReferences(&ost, 1));
  EXPECT_EQ(ost.lower_bound(2), ost.end());
  EXPECT_EQ(ost.lower_bound(2), ost.cend());
  set.insert(3);
  EXPECT_THAT(ost.insert(3), Pair(IteratorReferences(&ost, 3), true));
  // Tree is now {1, 3}
  EXPECT_EQ(ost.size(), 2);
  EXPECT_THAT(ost.find(0), IteratorAtEnd(&ost));
  EXPECT_THAT(ost.find(1), IteratorReferences(&ost, 1));
  EXPECT_THAT(ost.find(2), IteratorAtEnd(&ost));
  EXPECT_THAT(ost.find(3), IteratorReferences(&ost, 3));
  EXPECT_THAT(ost.find(4), IteratorAtEnd(&ost));
  EXPECT_NE(ost.select(0), ost.end());
  EXPECT_THAT(ost.select(0), IteratorReferences(&ost, 1));
  EXPECT_THAT(ost.select(1), IteratorReferences(&ost, 3));
  EXPECT_THAT(ost.select(2), IteratorAtEnd(&ost));
  EXPECT_THAT(ost.lower_bound(0), IteratorReferences(&ost, 1));
  EXPECT_THAT(ost.lower_bound(1), IteratorReferences(&ost, 1));
  EXPECT_THAT(ost.lower_bound(2), IteratorReferences(&ost, 3));
  EXPECT_THAT(ost.lower_bound(3), IteratorReferences(&ost, 3));
...

  {
    ...
    s2.insert(5);
    EXPECT_THAT(s2, ElementsAre(5));
    EXPECT_THAT(ost, ElementsAre(1, 3));
    std::swap(s2, ost);
    EXPECT_THAT(ost, ElementsAre(5));
    EXPECT_THAT(s2, ElementsAre(1, 3));
    ost.swap(s2);
    EXPECT_THAT(s2, ElementsAre(5));
    EXPECT_THAT(ost, ElementsAre(1, 3));
  }
...
TEST(OrderStatisticSetTest, Erase) {
  // Test with const key, sinc we'll need keys with const component for the map.
  std::set<const size_t> set;
  OrderStatisticSet<const size_t> ost;
  set.insert(1);
  ost.insert(1);
  ...
  set.insert(2);
  ost.insert(2);
  EXPECT_THAT(set.erase(set.find(1)), IteratorReferences(&set, 2));
  EXPECT_THAT(ost.erase(ost.find(1)), IteratorReferences(&ost, 2));
  EXPECT_EQ(set, ost);
...
size_t find_value_not_in_set(const std::set<const size_t>& set,
