...
//#include "googletestg/base/public/gunit.h"
...
// SINFAE tricks to determine if a comparison functor has is_transparent.  See
// the CheckPublicType tests for examples.
...
static std::pair<size_t, size_t> find_pair_not_in_map(
...
std::ostream&
RawOrderStatisticSet<Key, Value, Compare, NodeType>::PrintStructureForTest(
    std::ostream& out, std::function<std::string(const Value&)> kp) const {
  return Node::Print(out, root_, kp);
}

template <class Key, class Value, class Compare, class NodeType>
std::ostream& RawNode<Key, Value, Compare, NodeType>::Print(
