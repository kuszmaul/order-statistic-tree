...
// The base class itself.  The template arguments are
//
//   `Key`, `Value`, and `Compare`: Essentially the same as for `std::map`.
//
//   `NodeType`: All the real work of the tree is encapsulated in `NodeType`.
...
//
// The NodeType operations that modify the tree generally take a `NodeType`
// which is the root of the subtree, and return the new root as a `NodeType`.
// Thus, `NodeType::Insert` takes the root of the tree (as a `unique_ptr`) and a
// new value; inserts the value into the tree; possibly creates new nodes or
...
//
//   `root_ = NodeType::Insert(std::move(root_), new_value);`
//
// The actual arguments and return values for `NodeType::Insert` are slightly
// more complicated (so that a single method can handle `insert` and
// `insert_or_assign`).
//
...
template <class Key, class Value, class Compare, class NodeType>
class RawOrderStatisticSet {
 ...
 private:
  ...
  template <class K>
  using key_arg =
      typename KeyArg<IsTransparent<Compare>::value>::template type<K, Key>;
 ...
 private:
  ...
  template <bool is_const_iterator, class K = Key>
 ...
 protected:
  ...
  size_t erase(const key_arg<K> &k) {
    auto [new_root, rank_of_erased, new_successor, n_erased] =
 ...
 public:
  ...
  std::ostream &PrintStructureForTest(
      std::ostream &out, std::function<std::string(const Value &)> kp) const;
...
template <class Key, class Value, class Compare, class Node>
template <bool is_const_iterator>
class RawOrderStatisticSet<Key, Value, Compare, Node>::Iterator {
  friend Iterator<true>;

 public:
  using iterator_category = std::random_access_iterator_tag;
  using value_type =
      typename std::conditional_t<is_const_iterator, const Value, Value>;
  ...
  Value *operator->() { return &node_->value_; }
  const Value &operator*() const { return node_->value_; }
  Value &operator*() { return node_->value_; }
  const Value &operator[](std::ptrdiff_t idx) const { return *(*this + idx); }
  // Rank
  ...
  Iterator &operator+=(ptrdiff_t n) {
    ...
    // fail.
    if (n < 0) {
      CHECK_LE(-n, idx_);  // Crash OK
    } else {
      CHECK_LE(idx_ + n, tree_->size());  // Crash OK
    }
    idx_ += n;
    if (idx_ < tree_->size()) {
...
template <class Key, class Value, class Compare, class Node>
typename RawOrderStatisticSet<Key, Value, Compare, Node>::iterator
RawOrderStatisticSet<Key, Value, Compare, Node>::erase(iterator pos) {
  auto [new_root, new_idx, successor, n_erased] =
...
class RawNode {
 ...
 public:
    ...
    if (!n) {
      ...
      if (lessthan(Node::key(n->value_), Node::key(k))) {
        ...
        auto [new_root, inserted_idx, inserted_node, sub_did_insert] =
            Insert(std::move(n->right_), std::move(k), rank_so_far + left_size,
                   assign, lessthan);
  ...
  // Returns a tuple which can be parsed as
  //
  //  auto [new_root, rank_of_k, new_successor, n_erased] = Erase(...);
  //
  // where
  //
  //   `new_root` is the new root of the subtree that was previously rooted at
  ...
  static std::tuple<std::unique_ptr<Node>, size_t /*rank*/,
                    Node * /*new_successor*/, size_t /*n_erased*/>
  Erase(std::unique_ptr<Node> n, const Karg &k, size_t rank_so_far,
    ...
    if (n == nullptr) {
      return {nullptr, rank_so_far, nullptr, 0};
    } else if (lessthan(Node::key(n->value_), k)) {
      auto [newroot, rank, successor, n_erased] =
          Erase(std::move(n->right_), k, rank_so_far, successor_node, lessthan);
      ...
      return {DeleteNode(std::move(n)), rank_so_far, successor, 1};
    }
  }
 ...
 private:
  ...
  // Removes this from the tree, returning the new root.
  static std::unique_ptr<Node> DeleteNode(std::unique_ptr<Node> n) {
    DCHECK(n);
    if (n->left_) {
